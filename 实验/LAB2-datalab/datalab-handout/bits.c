...
{
  return (x + ((x >> 31) & ((1 << n) + (~0)))) >> n; //x需要加上一个偏移量m = x >= 0 ? 0 : 2 ^ k - 1
}
...
{
  unsigned sign = x & 0x80000000, absx = x, shiftleft = 1, exp, frac, carry;
  if (x == 0)
    return 0;
  if (sign)
    absx = -x;
  ...
  absx <<= 1;
  frac = (absx >> 9) & 0x007fffff;
  exp = (159 - shiftleft) << 23;
  if ((absx & 0x000001ff) > 0x00000100)
    carry = 1;
  else if ((absx & 0x000003ff) == 0x00000300)
    carry = 1;
  else
    carry = 0;
  return sign + exp + frac + carry;
}
